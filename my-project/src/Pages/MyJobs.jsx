...
const MyJobs = () => {
    ...
    useEffect(() => {
        setIsLoading(true)
        fetch(`http://localhost:5000/myJobs/akshatgupta786@gmail.com`).then(res => res.json()).then(data =>{
            setJobs(data);
            setIsLoading(false);
        }); 
    },[searchText]);
    ...
    const handleSearch = () =>{
        const filter = jobs.filter((job)=>job.jobTitle.toLowerCase().indexf(searchText.toLowerCase())!==-1);
        //console.log(filter);
        setJobs(filter);
        setIsLoading(false);
    };
  ...
  return (
    ...
    <div className="search-box p-2 text-center mb-2">
        ...
        type="text" name="search" id ="search" className="py-2 pl-3 border focuse:outline:none lg:w-6/12 mb-4 w-full"/>
  ...
  <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
    <div className="rounded-t mb-0 px-4 py-3 border-0">
      <div className="flex flex-wrap items-center">
        ...
        <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right"><Link to="/post-job">
    ...
    isLoading ? (<div className="flex items-center justify-center h-20"><p>loading.....</p></div>):(<tbody>
        {
            ...
            <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
              <button className="bg-red-700 py-2 px-6 text-white rounded-sm">Edit<Link to={`/edit-job/${job?._id}`}></Link> </button>
            </td>
            <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
